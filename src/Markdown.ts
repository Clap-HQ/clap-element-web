...
const ALLOWED_HTML_TAGS = ["sub", "sup", "del", "s", "u", "br", "br/", "table", "thead", "tbody", "tr", "th", "td"];
...
function isAllowedHtmlTag(node: commonmark.Node): boolean {
    ...
    // Allow GFM table HTML blocks (generated by convertGfmTablesToHtml preprocessor)
    if (/^<table>.*<\/table>\s*$/.test(node.literal)) {
        return true;
    }

    // Allow table cell tags with style attribute for text-align
    if (/^<(th|td) style="text-align:(left|center|right)">$/.test(node.literal)) {
        return true;
    }

    // Allow task list checkbox input tags (generated by convertGfmTaskListToHtml preprocessor)
    if (/^<input type="checkbox"(?: checked)?(?: disabled)?\s*\/?>$/.test(node.literal)) {
        return true;
    }
